...
3. **Удалить задачу** (когда реализовано): действие удаления для выбранной задачи (кнопка, иконка и т.п.) — контракт дополнится после появления функции.

## localStorage

- **Ключ**: `todo-app-tasks`.
- **Формат**: JSON-массив объектов задач (поля: id, text, completed, order). E2E могут читать этот ключ после перезагрузки страницы для дополнительной проверки персистентности или полагаться только на отображение в UI.
